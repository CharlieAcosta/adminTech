...
- `01-views/panel.php` y `01-views/layout/navbar_layout.php` muestran `Seguimiento de obra` al perfil `Administrativo` cuando existe al menos una OC habilitada para su bandeja, sea pendiente o ya cargada/observada. En el panel principal, el modulo conserva el titulo `Seguimiento de obra` y el icono de seguimiento, pero para ese perfil el badge visible sigue mostrando solo pendientes calculadas con `contarOrdenesCompraPendientes()`; los perfiles tecnicos y administradores conservan los contadores tecnicos de vencidas/programadas/reprogramadas y suman el badge `OC Pend.` cuando existen OC pendientes.
