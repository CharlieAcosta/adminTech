...
- `01-views/auditoria_configuracion.php` es el subpanel exclusivo de `Super Administrador` para accesos administrativos sensibles y agrupa `Auditoria`, `Configuracion mail presupuestos`, `Perfiles`, `Dump` y `Migraciones`. No se muestra como boton/modulo dentro del panel principal; el acceso queda concentrado en los atajos de cabecera existentes para conservar las mismas rutas y opciones de visualizacion sin duplicar el modulo.
