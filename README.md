...
- `04-modelo/ordenCompraWorkflowModel.php` centraliza el primer paso del circuito OC dentro de seguimiento: perfiles con seguimiento completo (`Super Administrador`, `Administrador`, `Tecnico`, `Tecnico Administrativo`), perfil `Administrativo` como acceso solo OC cuando el circuito esta habilitado, edicion de OC para perfiles administrativos y estado calculado `no_habilitada`/`pendiente` desde el estado comercial `APROBADO`. Hasta que exista persistencia real de OC, el listado no inventa numero ni estados cargados: muestra `OC Pendiente` cuando el presupuesto esta aprobado y `No habilitada` en el resto.
