...
- PDFs respaldatorios de OC: `uploads/presupuestos/{id_presupuesto}/ordenes_compra/`.
...
- `04-modelo/ordenCompraWorkflowModel.php` centraliza el circuito OC dentro de seguimiento: perfiles con seguimiento completo (`Super Administrador`, `Administrador`, `Tecnico`, `Tecnico Administrativo`), perfil `Administrativo` como acceso solo OC cuando el circuito esta habilitado, edicion de OC para perfiles administrativos y estado calculado `no_habilitada`/`pendiente` desde el estado comercial `APROBADO`. Cuando no existe OC activa, el listado no inventa numero ni estados cargados: muestra `OC Pendiente` si el presupuesto esta aprobado y `No habilitada` en el resto.
- `03-controller/presupuestos_guardar.php`, `04-modelo/presupuestoGeneradoModel.php`, `04-modelo/visitaModel.php` y `04-modelo/visitaPresupuestoEventoModel.php` implementan el comando backend `generarPresupuestoDesdeVisita`: requiere sesion autenticada, usa el perfil de seguimiento completo, construye el primer `Borrador` desde la visita persistida, serializa por advisory lock MySQL sobre `id_previsita`, y confirma atomicamente el presupuesto junto con el evento `VISITA_CONGELADA_POR_GENERACION_PRESUPUESTO` de origen `GENERACION`.
- `01-views/seguimiento_form.php` expone `VISITA_CONGELADA_POR_PRESUPUESTO` desde `visita_presupuesto_eventos` y `07-funciones_js/accordionVisita.js` limita el congelamiento visual exclusivamente a Visita: el boton `Generar Presupuesto` llama al comando backend, no envia snapshot del navegador y converge por recarga al Presupuesto persistido antes de dejar la Visita en solo consulta.
- `03-controller/presupuestosController.php` y `07-funciones_js/presupuestosAcciones.js` mantienen sincronizada la columna `Orden de compra` del listado: al registrar un estado comercial desde el historial se recarga la grilla por el circuito AJAX existente para que `APROBADO` sin OC pase a `OC Pendiente` sin refresh manual. El perfil `Administrativo` no recibe acciones tecnicas de visita/presupuesto, pero si consulta, edicion de pre-visita y un icono propio de gestion OC que abre `seguimiento_form.php`. `Administrador` y `Super Administrador` conservan todas las acciones del seguimiento normal y suman un icono OC en la columna Acciones cuando la OC esta habilitada o tiene estado real activo, usando `seguimiento_form.php?acci=e&id={id_previsita}&oc=1#collapse4_OC`.
- `01-views/panel.php` y `01-views/layout/navbar_layout.php` muestran `Seguimiento de obra` al perfil `Administrativo` cuando existe al menos una OC habilitada para su bandeja, sea pendiente o ya cargada/observada. En el panel principal, el modulo conserva el titulo `Seguimiento de obra` y el icono de seguimiento; para ese perfil, el badge de pendientes calculado con `contarOrdenesCompraPendientes()` solo se muestra cuando el valor es mayor a cero. Los perfiles tecnicos y administradores conservan los contadores tecnicos de vencidas/programadas/reprogramadas y suman el badge `OC Pend.` cuando existen OC pendientes.
- Para el perfil `Administrativo`, el acceso rapido de cabecera puede mantener la referencia visual a OC, pero el panel de modulos no muta el boton de `Seguimiento de obra`. Al ingresar, `01-views/seguimiento_de_obra_listado.php` usa la pantalla como bandeja administrativa con titulo `Previsita & OC | Listado`; otros perfiles conservan `Seguimiento de obra`.
- `01-views/seguimiento_de_obra_listado.php` usa el listado `Previsita & OC | Listado` como bandeja para `Administrativo`: oculta los filtros tecnicos de visita/presupuesto, muestra `Agregar` para cargar una pre-visita, mantiene filtros de tiempo y arranca en `30 dias`. El universo inicial combina OC pendientes y pre-visitas operables (`Programada`, `Reprogramada`, `Vencida`), ordenando primero OC pendientes y despues pre-visitas. Los filtros administrativos `Pendientes`, `Cargadas`, `Todas OC` y `Previsita` se aplican server-side combinados con el rango temporal; `Pendientes` equivale a presupuestos `APROBADO` con estado calculado `OC Pendiente`, `Cargadas` no inventa datos, `Todas OC` no muestra `No habilitada` y `Previsita` limita la bandeja a pre-visitas operables. En Acciones, `Administrativo` ve solo consulta, edicion de pre-visita y acceso OC; los bloques tecnicos del formulario quedan fuera de su interfaz.
- Para `Tecnico`, `Tecnico Administrativo`, `Administrador` y `Super Administrador`, los filtros `Pendientes`/`Cargadas`/`Todas OC` de la columna OC se muestran como filtros adicionales opcionales sobre el listado normal de seguimiento: no tienen valor activo por defecto, no cambian el rango inicial ni la base temporal tecnica, y se limpian con `Sin filtro OC`.
- Los rangos de tiempo de la bandeja administrativa `Previsita & OC` se aplican sobre el universo mixto: pre-visitas por `previsitas.log_alta` y OC por la fecha de aprobacion comercial disponible, con respaldo en `previsitas.log_alta` si no hay historial. La carga inicial mantiene activos `Pendientes`, `Previsita` y `30 dias`.
- Paso 2 de OC agrega la persistencia base sin UI: `11-migraciones_sql/2026-05-09-A_ordenes_compra.sql` crea `ordenes_compra` para datos principales, comerciales, entrega, facturacion, seguridad, metadatos PDF y auditoria, sin tabla de items ni endpoint de carga. `04-modelo/ordenCompraModel.php` centraliza lecturas reales de OC y define OC activa como estado `cargada` u `observada`; `anulada` no cuenta como activa, por lo que un presupuesto `APROBADO` con solo OC anuladas vuelve a calcularse como `OC Pendiente`.
- Paso 3 de OC agrega backend JSON sin UI final en `03-controller/ordenesCompraController.php`: acciones `obtener_orden_compra`, `guardar_orden_compra`, `actualizar_orden_compra` y `cambiar_estado_orden_compra`. La escritura queda restringida a `Administrativo`, `Administrador` y `Super Administrador`; `Tecnico` y `Tecnico Administrativo` solo consultan. El alta exige presupuesto `APROBADO`, bloquea una segunda OC activa por presupuesto, persiste solo estados `cargada`/`observada`/`anulada` y deja la obligatoriedad del PDF para el paso de carga de archivo.
- Paso 4 de OC convierte el acordeon `collapse4_OC` de `01-views/seguimiento_form.php` en formulario real conectado al backend de Paso 3: carga datos con `obtener_orden_compra`, crea con `guardar_orden_compra`, actualiza con `actualizar_orden_compra` y permite marcar `observada`/`anulada` con `cambiar_estado_orden_compra`. El formulario queda editable solo para `Administrativo`, `Administrador` y `Super Administrador`; los perfiles tecnicos lo consultan en solo lectura.
- Los botones operativos del acordeon OC (`Guardar OC`, `Marcar observada`, `Anular OC` y `Volver`) viven dentro del cuerpo del propio acordeon, al pie del formulario, siguiendo el patron visual del resto de acordeones; la cabecera queda reservada para abrir/cerrar la seccion y mostrar el estado visual.
- La cabecera del acordeon OC muestra `Intervino` cuando existe una OC activa real, usando la auditoria minima de `ordenes_compra`: `id_usuario_alta`/`created_at` para `Cargar OC` y `id_usuario_modificacion`/`updated_at` para `Actualizar OC`; como no hay historial propio de OC todavia, el popover se limita a esos hitos disponibles.
- Cuando el acordeon OC recibe una OC activa real (`cargada` u `observada`), `01-views/seguimiento_form.php` pinta la cabecera de OC en verde, colapsa inicialmente `collapse4_OC` y habilita/expande el acordeon siguiente `Pedido de materiales`; si la OC vuelve a pendiente, anulada o no habilitada, el pedido de materiales queda oculto nuevamente.
- El acordeon OC solo se renderiza cuando el circuito ya esta habilitado para carga/consulta de OC; en etapa Pre-visita o con estado calculado `no_habilitada` permanece oculto completo, incluida la cabecera.
- El acordeon `Pedido de materiales` separa `Materiales presupuestados` desde las tareas del presupuesto generado (`presupuesto_tarea_material`) y `Materiales agregados` desde el alta manual con el mismo selector Select2 usado en Visita; esos adicionales se persisten en `materiales_visita` asociados a la previsita. El selector de alta manual excluye materiales ya presupuestados y materiales ya agregados. Ambas tablas muestran `Cantidad inicial` en verde y `Agregado` en rojo; la accion `+` por fila suma una unidad al material de esa misma fila en `Agregado`, y la accion `-` quita primero de `Agregado` y luego de `Cantidad inicial`, con limite cero y sin interactuar con el Select2. En `Materiales agregados`, el tachito quita la fila completa, compensa el saldo persistido y devuelve el material al Select2. Este acordeon lo ven y editan perfiles con seguimiento completo (`Tecnico`, `Tecnico Administrativo`, `Administrador`, `Super Administrador`), separado del permiso de edicion OC del perfil `Administrativo`. La vista no implementa todavia items OC, historial propio ni una tabla formal nueva de pedido de materiales.
- Panel de analitica - maqueta visual read-only para validacion con cliente. Mantiene el acceso desde la columna `Acciones` y el modal tecnico `modalDashboardSeguimiento`; los datos actuales de identificacion, KPIs, duracion, materiales, jornales y autorizaciones estan hardcodeados y no provienen de backend, AJAX ni consultas SQL.

### Pedido de Materiales - Reglas de botones e iconos

- **BRP - Realizar pedido**
- **BRP-001:** El boton permanece visible siempre. Mientras todas las cantidades de la columna Pedido sean `0`, se muestra gris y deshabilitado. Cuando al menos una cantidad de Pedido sea mayor que `0`, se habilita y se muestra verde. Aplica a Materiales presupuestados y Materiales agregados.
- **BRP-002:** Cuando el BRP esta habilitado y el usuario hace clic, se muestra un SweetAlert normal de confirmacion con icono success. El modal incluye una tabla con las columnas **Descripcion** y **Cantidad**, listando solo materiales con cantidad de Pedido mayor que `0`. La descripcion se muestra solo hasta el primer caracter `|`. El modal incluye botones **Confirmar** y **Cancelar**. Confirmar inicia el guardado previo del snapshot y la confirmacion backend formal definida en **PM-PED-006**; el ciclo visual solo avanza si el backend responde correctamente.
...
- **BRP-004 / PM-PED-001:** Al confirmar un pedido, su columna queda congelada e inmutable y, mientras no sea el Pedido #5, se crea la siguiente columna en `0`. **Inicial** no se reinicia y **Solicitado** continua siendo la suma acumulada de todas las columnas Pedido. BRP, el SweetAlert y las reglas de autorizacion trabajan exclusivamente con el pedido activo. Al iniciar cada nuevo ciclo se restablece la autorizacion visual a `Sin solicitud`, se limpian datos temporales del ciclo anterior y se habilitan nuevamente las acciones de cantidad. El avance visual ocurre despues de la confirmacion backend formal y se conserva mediante el snapshot posterior.
...
- **PM-AUT-003:** Toda fila pendiente mantiene su estado en la columna **Autorizacion**. Los controles **Autorizar** y **Rechazar** se muestran en **Acciones** solo a `Super Administrador` y `Administrador`; `Tecnico`, `Técnico` y `Tecnico Administrativo` pueden generar y guardar solicitudes pendientes, pero no decidirlas. La decision se procesa exclusivamente en backend con el usuario y perfil de sesion; no existe accion de revertir.
...
- **PM-AUT-007:** La autorizacion de excedentes de Pedido de Materiales es una decision backend formal. Solo `Super Administrador` y `Administrador` pueden autorizar o rechazar; los perfiles tecnicos pueden solicitar, pero no decidir, y `Administrativo` no accede al modulo. `pedido_materiales_autorizaciones` registra la decision vigente por previsita, numero de pedido y clave estable de fila, junto con cantidades, hash de contexto, usuario autorizante, fecha y motivo opcional. El snapshot puede guardar solicitudes `pendiente`, pero no puede convertirlas en `autorizada` o `rechazada` sin una decision formal registrada, ni cambiar las cantidades cubiertas por esa decision. La confirmacion backend exige una autorizacion formal vigente para todo agregado positivo o excedente presupuestado.
- **PM-PED-003 / BRP-005:** El flujo de Pedido de Materiales permite hasta cinco columnas de pedido: **Pedido #1** a **Pedido #5**. Al confirmar un pedido del #1 al #4, la columna confirmada queda congelada e inmutable y se crea la siguiente columna en `0`, que pasa a ser el pedido activo y hereda todas las reglas de cantidades, autorizacion, BRP, SweetAlert, materiales agregados y tooltips. Al confirmar **Pedido #5**, no se crea Pedido #6, el flujo queda finalizado y el BRP permanece visible, gris y deshabilitado. Cada confirmacion pasa por un punto central preparado para incorporar acciones y eventos adicionales en una etapa posterior. La logica del ciclo sigue siendo frontend, pero el estado ya puede persistirse mediante snapshots del modulo.
...
- **PM-PED-005 / BRP-006:** El modulo permite guardar un snapshot del estado actual del Pedido de Materiales mediante el boton **Guardar pedido**, ubicado junto a **Realizar pedido**. El guardado persiste columnas visibles, cantidades por pedido, pedido activo, estado finalizado, materiales presupuestados/agregados y estados de autorizacion relevantes, registrando fecha, hora, usuario y tipo de accion (`guardar` o `realizar`). El boton solo se habilita si existen cambios pendientes. **Guardar pedido** no confirma pedidos ni avanza ciclos; solo conserva el estado para retomarlo. **Realizar pedido** guarda snapshot antes de continuar con la confirmacion del pedido activo y vuelve a guardar el nuevo estado luego del avance de ciclo para que el modulo pueda retomarse tal como quedo.
- **PM-PED-006:** Al ejecutar **Realizar pedido**, el pedido activo se confirma en backend como entidad propia antes de avanzar visualmente al siguiente ciclo. La confirmacion se guarda en `pedido_materiales_pedidos` y `pedido_materiales_pedido_detalles`, con cabecera unica por `id_previsita + numero_pedido`, referencia al presupuesto y la Orden de Compra habilitante, usuario de sesion, fecha backend y detalle congelado de las cantidades confirmadas. Esta confirmacion es idempotente: reintentar el mismo pedido con el mismo detalle no lo duplica; si el detalle difiere, se rechaza el avance por conflicto. PM-PED-006 no genera PDF ni envia correo durante la confirmacion; la generacion posterior del documento corresponde a PM-MAIL-002 y el envio posterior a PM-MAIL-003.
- Los avisos transitorios de exito y error del guardado y actualizacion de Pedido de Materiales usan el toast central del sistema definido en `07-funciones_js/funciones.js`, con cierre automatico. SweetAlert2 queda reservado para modales de confirmacion o decision, como la confirmacion de **Realizar pedido** y los flujos de autorizacion.
- El snapshot de Pedido de Materiales toma su contexto desde `window.SEGUIMIENTO_PEDIDO_MATERIALES`, incluyendo `id_previsita` y la URL de guardado, para evitar depender nominalmente de la configuracion JS de Orden de Compra.
- Las cantidades del snapshot se normalizan como valores numericos antes de volver al frontend para evitar interpretar decimales SQL como `1200000000.0000` bajo la misma logica usada para textos visuales con separadores de miles.
- Persistencia tecnica del snapshot de Pedido de Materiales:
  - SQL versionado: `11-migraciones_sql/2026-07-07-A_pedido_materiales_snapshots.sql`.
  - Tablas nuevas: `pedido_materiales_snapshots` (cabecera por `id_previsita`) y `pedido_materiales_snapshot_detalles` (detalle por fila/material).
  - Endpoint: `03-controller/pedidoMaterialesController.php`.
  - Modelo: `04-modelo/pedidoMaterialesSnapshotModel.php`.
  - Seguridad: el usuario del guardado se toma exclusivamente de `$_SESSION['usuario']['id_usuario']`; no se acepta desde POST/JSON.
  - Convivencia con legado: `materiales_visita` sigue registrando movimientos unitarios del flujo actual, mientras el snapshot persiste el estado completo reanudable del modulo.
- Persistencia tecnica de autorizaciones de Pedido de Materiales:
  - SQL versionado: `11-migraciones_sql/2026-07-31-A_pedido_materiales_autorizaciones_backend.sql`.
  - Tabla nueva: `pedido_materiales_autorizaciones`, independiente del detalle reemplazable del snapshot y unica por `id_previsita + numero_pedido + clave_fila`.
  - Endpoint: accion `resolver_autorizacion_pedido_materiales` de `03-controller/pedidoMaterialesController.php`.
  - Modelo: `04-modelo/pedidoMaterialesAutorizacionesModel.php`, que centraliza identidad de fila, contexto de cantidades, auditoria, proteccion del snapshot y validacion previa a la confirmacion.
  - Seguridad: `id_usuario_autorizacion` y el perfil se resuelven exclusivamente desde `$_SESSION`; el endpoint no acepta usuario ni perfil desde POST/JSON.
- Persistencia tecnica de pedidos confirmados:
  - SQL versionado: `11-migraciones_sql/2026-07-29-A_pedido_materiales_pedidos_confirmados.sql`.
  - Tablas nuevas: `pedido_materiales_pedidos` (cabecera unica por previsita y numero de pedido) y `pedido_materiales_pedido_detalles` (detalle inmutable con cantidades mayores que cero).
  - Endpoint: accion `confirmar_pedido` en `03-controller/pedidoMaterialesController.php`.
  - Modelo: `04-modelo/pedidoMaterialesPedidosModel.php`.
  - Habilitacion: una confirmacion nueva exige una Orden de Compra activa `cargada` u `observada` para el presupuesto mas reciente de la previsita.
  - Idempotencia: la clave unica se complementa con un hash canonico del detalle; un reintento identico recupera la confirmacion existente y uno divergente devuelve conflicto.
  - Reintento historico: un pedido ya confirmado con el mismo hash se recupera de forma idempotente aunque su Orden de Compra haya dejado de estar activa; la validacion de OC habilitante aplica exclusivamente al alta de una confirmacion nueva.
- Persistencia tecnica del PDF de pedidos confirmados:
  - SQL versionado: `11-migraciones_sql/2026-07-29-B_pedido_materiales_pedido_documentos.sql`.
  - Tabla nueva: `pedido_materiales_pedido_documentos`, con un documento vigente de tipo `pedido_materiales_pdf` por pedido confirmado, ruta controlada, MIME, tamano, hash y usuario de sesion que lo genero.
  - Fuente: `pedido_materiales_pedidos` y `pedido_materiales_pedido_detalles`; las cantidades no se reconstruyen desde snapshot, DOM, `materiales_visita` ni frontend.
  - Modelo: `04-modelo/pedidoMaterialesPdfModel.php`, que genera un PDF A4 multipagina desde backend sin depender del navegador y devuelve rutas/metadatos reutilizables por el futuro envio.
  - Datos visibles: el numero de presupuesto se obtiene del documento asociado a la aprobacion comercial vigente (`presupuesto_historial_comercial.id_documento_emitido` hacia `presupuesto_documentos_emitidos.nombre_archivo`) y se presenta con el mismo codigo del PDF aprobado, nunca con `id_presupuesto` como numero principal. Si ese documento confiable no esta disponible, se informa `No disponible`. El usuario del pedido se toma de `pedido_materiales_pedidos.id_usuario_confirmacion` y se completa con la tabla `usuarios`.
  - Identificacion y listado: los PDFs muestran el identificador visible como `ID`, no como `Previsita`; los archivos nuevos se nombran con `ID_{id}` e `INTERNO_{id_pedido}`. La pantalla lista por `numero_pedido` los PDFs existentes usando exclusivamente `pedido_materiales_pedidos` y `pedido_materiales_pedido_documentos`, y descarga por ID de pedido sin aceptar rutas del cliente.
  - Archivos: `uploads/pedido_materiales/{id_previsita}/pedidos/`, con nombre deterministico construido exclusivamente desde IDs numericos del pedido confirmado.
  - Endpoint: acciones `generar_pdf_pedido` y `descargar_pdf_pedido` de `03-controller/pedidoMaterialesController.php`; la descarga recibe solo el ID del pedido, resuelve la ruta registrada en DB y valida que permanezca dentro del directorio permitido.
- Persistencia tecnica del envio por correo:
  - SQL versionado: `11-migraciones_sql/2026-07-29-C_pedido_materiales_pedido_envios.sql`.
  - Tabla nueva: `pedido_materiales_pedido_envios`, con una fila vigente por pedido y tipo, estados `pendiente`, `procesando`, `enviado`, `simulado` o `error`, cantidad de intentos, destinatarios resueltos desde configuracion, documento adjunto, ultimo error saneado y usuario de sesion.
  - Modelo: `04-modelo/pedidoMaterialesEnviosModel.php`, que reclama cada intento en una transaccion corta, reutiliza o genera el PDF backend y registra el resultado sin mantener locks durante SMTP. En modo real valida la configuracion activa y envia por PHPMailer; en simulacion no carga el transporte SMTP.
  ...
  - Idempotencia: un envio `enviado` responde como ya realizado sin repetir SMTP, un envio `simulado` responde `ya_simulado=true` sin incrementar intentos ni duplicar la fila, `procesando` bloquea un segundo intento concurrente y `error` permite reintentar manualmente.
  - Simulacion local: en un entorno no productivo (`APP_ENV=development`, `local`, `test`, `qa` o equivalente) puede inyectarse `ADMINTECH_PEDIDO_MATERIALES_MAIL_SIMULACION=1` en el ambiente del servidor PHP. El modo genera o reutiliza el PDF, registra `estado=simulado`, conserva `fecha_envio=NULL`, no usa SMTP ni PHPMailer y nunca se activa por datos enviados desde el cliente. La variable se ignora cuando el ambiente es productivo.
  ...
  - Estado en pantalla: la seccion **PDFs generados** muestra tambien el estado persistido del envio por pedido. El boton manual se reserva para pedidos no enviados o con `error`; cuando el estado es `enviado` o `simulado`, la UI informa **Correo enviado** o **Correo simulado** y evita presentarlo como accion pendiente.
  ...
  - Pendientes posteriores: prueba contra un SMTP real o sandbox autorizado, mejoras de plantilla, monitoreo y reintentos avanzados y CSRF.
...
- Paso 5 de OC conecta el boton `OC` del historial comercial de presupuesto en `07-funciones_js/presupuestosAcciones.js` con el acordeon real: si la vista ya contiene `#collapse4_OC`, cierra el modal de historial, cierra otros acordeones, abre el acordeon OC y enfoca el bloque; si el usuario esta fuera del formulario, navega a `seguimiento_form.php?acci=e&id={id_previsita}&oc=1#collapse4_OC`. En `01-views/seguimiento_form.php`, el parametro `oc=1` tiene prioridad de apertura exclusiva para que no queden abiertos Pre-visita, Visita ni Presupuesto. Ya no muestra alerta de funcionalidad en desarrollo.
- Paso 6 de OC deja la columna `Orden de compra` como badge definitivo del listado: `No habilitada` sigue siendo calculado cuando el presupuesto no esta aprobado, `OC`/`Pendiente` se muestra en dos lineas para presupuestos aprobados sin OC activa, y cuando existe OC activa se usa el `numero_oc` real de `ordenes_compra` junto con el estado persistido `Cargada` u `Observada`. Las OC `anulada` no cuentan como activas y hacen que un presupuesto aprobado vuelva a verse como `OC Pendiente`; no se inventan numeros ni estados.
- Paso 7 de OC exige PDF respaldatorio para crear o conservar una OC activa: `guardar_orden_compra` bloquea altas sin archivo, `actualizar_orden_compra` permite editar sin reemplazar solo si la OC ya tiene PDF y exige archivo para registros legacy sin respaldo. Los PDF se validan por extension `.pdf`, firma real `%PDF`, MIME `application/pdf`, tamaño mayor a cero y maximo 5 MB; se guardan en `uploads/presupuestos/{id_presupuesto}/ordenes_compra/` y se sirven por `descargar_pdf_orden_compra` desde `03-controller/ordenesCompraController.php`. Los perfiles administrativos pueden cargar/reemplazar; `Tecnico` y `Tecnico Administrativo` solo consultan/descargan.
- El bloque `Documentacion OC` permite consultar el presupuesto aprobado vigente sin exponer rutas de `uploads`: resuelve el ultimo evento comercial `APROBADO` del modo activo, exige que su documento asociado tenga un envio exitoso en ese mismo modo y sirve el PDF inline desde `03-controller/presupuestos_guardar.php` con validacion de sesion, perfil, presupuesto, previsita, documento y ruta fisica. La accion de consulta esta disponible para `Administrativo`, `Administrador`, `Super Administrador`, `Tecnico` y `Tecnico Administrativo`, abre una pestaña nueva y no modifica la OC.
- La integracion OC -> Clientes se limita al alta automatica opcional de cliente nuevo al crear una OC: `guardar_orden_compra` verifica el CUIT normalizado contra `clientes`, no actualiza clientes existentes y, si no encuentra coincidencia, devuelve `requires_cliente_confirmation=true` para que la UI muestre un modal SweetAlert2 `warning` con las opciones `Si, dar de alta cliente` y `No, solo guardar OC`. Si el usuario confirma, se crea el cliente con los campos disponibles del circuito compatibles con el maestro `clientes` mediante consultas preparadas; si rechaza, la OC se guarda sin tocar Clientes. La deteccion es exclusivamente por CUIT y la creacion queda restringida a perfiles que ya pueden editar OC (`Administrativo`, `Administrador`, `Super Administrador`).
- Paso 9A/9B de OC agrega campos administrativos adicionales a `ordenes_compra` para nombre de fantasia y direccion fiscal del proveedor, direccion de obra alternativa, contactos de compras y obra/mantenimiento, URLs de portales de facturacion e ingreso a obra, caucion y poliza RC. Los portales guardan solo URLs, sin usuarios ni contrasenas; se mantienen fuera de alcance CTA CTE, FC, NC, pagos, retenciones, items e historial propio. Esos campos son opcionales para compatibilidad de despliegue: si la tabla base existe pero todavia no se aplico la migracion 9A, el circuito OC base no se invalida ni arma SQL contra columnas inexistentes.
- Paso 9C expone esos campos adicionales en el acordeon real de OC dentro de `01-views/seguimiento_form.php`, manteniendo PDF obligatorio arriba, carga/descarga/reemplazo de PDF sin cambios, `Administrativo`/`Administrador`/`Super Administrador` editables y perfiles tecnicos en solo lectura.
- El pulido visual del acordeon OC mantiene el PDF obligatorio arriba, presenta avisos internos como alert cards compactas, mejora la lectura de contactos/switches y confirma `Anular OC` con SweetAlert2 cuando esta disponible; no cambia permisos, backend ni reglas de PDF.
- En el formulario OC, `Documentacion OC` queda disponible para cargar/reemplazar PDF solo en perfiles con edicion (`Administrativo`, `Administrador`, `Super Administrador`); el resto de las secciones de datos se bloquea funcional y visualmente hasta que exista un PDF guardado o se seleccione un PDF valido en la UI. Al seleccionar o quitar PDF se recalculan `disabled`/`readonly` y overlays sin cambiar permisos; los perfiles tecnicos siguen siempre en solo lectura. El guardado sigue pasando por backend, pero la UI advierte con SweetAlert2 si falta PDF o datos principales minimos.
- En el formulario de Clientes, `04-modelo/partidosModel.php` y `04-modelo/localidadesModel.php` deben devolver siempre un array, incluso sin resultados, para que los selects dependientes de provincia/partido no emitan warnings al editar clientes creados desde OC con ubicacion parcial o sin listas asociadas.
...
- `04-modelo/presupuestoDocumentosEmitidosEnviosModel.php` arma el cuerpo del mail al cliente con saludo formal al equipo de la razon social, adjunta el numero de presupuesto sin repetir el cliente en esa linea y cierra con `Saludos cordiales`.
- El envio de un documento emitido de presupuesto siempre queda auditado en el `Historial de presupuesto` cuando el transporte no falla, aunque el PDF enviado no sea la ultima emision vigente. Solo la ultima emision puede iniciar/impactar el estado comercial `Enviado`; los envios de documentos antiguos se registran como evento historico sin cambiar el estado comercial activo y el modal los marca en rojo. Los circuitos asociados a documentos emitidos anteriores permanecen visibles en gris tenue para conservar trazabilidad sin mezclarlos con el circuito vigente.
...
- `01-views/auditoria_configuracion.php` es el subpanel exclusivo de `Super Administrador` para accesos administrativos sensibles y agrupa `Auditoria`, `Configuracion mail presupuestos`, `Configuracion mail pedido materiales`, `Perfiles`, `Dump` y `Migraciones`. No se muestra como boton/modulo dentro del panel principal; el acceso queda concentrado en los atajos de cabecera existentes para conservar las mismas rutas y opciones de visualizacion sin duplicar el modulo.
- El modulo `Migraciones` registra ejecuciones en la tabla `migraciones`, versionada por `11-migraciones_sql/2026-05-10-A_migraciones_control.sql`; `04-modelo/migracionesModel.php` conserva un fallback centralizado `CREATE TABLE IF NOT EXISTS` solo para bootstrap defensivo del propio panel.
- El login usa `SESION_TIME = 10800` (3 horas) para la sesion por inactividad. La opcion `Recordarme` no guarda contrasenas: crea una cookie HTTP-only con selector/validador y persiste solo el hash del validador en `usuario_remember_tokens` (`11-migraciones_sql/2026-04-29-A_usuario_remember_tokens.sql`); al cerrar sesion se revoca el token actual.
...
- **PM-MAIL-001:** La configuracion de correo para los pedidos de materiales se administra desde `01-views/configuracion_mail_pedido_materiales.php`, una pantalla exclusiva de `Super Administrador` apoyada en `04-modelo/pedidoMaterialesConfigCorreoModel.php`. La configuracion persiste en `pedido_materiales_config_correo` mediante `11-migraciones_sql/2026-07-12-A_pedido_materiales_config_correo.sql` y define servidor SMTP, remitente, destinatarios `TO`/`CC`/`CCO`, asunto base, cuerpo base y estado activo/inactivo. Las credenciales SMTP no se muestran en pantalla, no viajan por JSON y se almacenan cifradas reutilizando el mismo secreto central (`MAIL_PRESUPUESTOS_SECRET` / `ADMINTECH_MAIL_SECRET` o `/admintech_secrets/mail_secret.php`) ya usado por el mail comercial de presupuestos.
- **PM-MAIL-002:** Los pedidos de materiales confirmados pueden generar un PDF desde backend tomando como fuente `pedido_materiales_pedidos` y `pedido_materiales_pedido_detalles`. El PDF representa el detalle congelado del pedido confirmado, se registra en `pedido_materiales_pedido_documentos`, se guarda bajo `uploads/pedido_materiales/{id_previsita}/pedidos/` y queda preparado para ser adjuntado en PM-MAIL-003. La UI habilita una descarga minima despues de una confirmacion backend correcta. Esta etapa no envia correo.
- **PM-MAIL-003:** Al completar **Realizar pedido**, el sistema envia un correo del pedido de materiales confirmado usando la configuracion activa de `pedido_materiales_config_correo` y adjuntando el PDF backend de PM-MAIL-002. El envio se registra en `pedido_materiales_pedido_envios`, es idempotente por pedido/tipo y evita duplicados: si ya fue enviado, no vuelve a mandar automaticamente; si falla, conserva el error y permite reintento manual. Esta etapa no modifica la confirmacion backend ni el snapshot.
- En esa configuracion de correo de Pedido de Materiales, `smtp_password` se persiste cifrado en un campo `TEXT` para evitar truncamientos del payload protegido antes de una futura integracion con el envio real.
- En la misma configuracion, el puerto SMTP se valida en backend como entero obligatorio entre `1` y `65535`; la UI solo sugiere ese rango con `min`/`max`, pero no hay autocorreccion silenciosa del valor ingresado.
- La pantalla de configuracion de mail de Pedido de Materiales reutiliza el patron visual AdminLTE del panel de Super Administrador y los avisos transitorios del sistema mediante `mostrarExito(...)` / `mostrarError(...)` de `07-funciones_js/funciones.js`; no expone la contrasena ni dispara pruebas SMTP desde la pantalla. El envio real se inicia desde el pedido confirmado mediante PM-MAIL-003.
...
- `07-funciones_js/presupuestosAcciones.js` actualiza dinamicamente la etiqueta de la columna `Presupuesto` resolviendo el indice por encabezado visible, no por posicion fija, para que cambios como `Requerimiento tecnico` no desplacen el badge hacia `Hora` despues de un envio real.
...
### Verificacion rapida de importacion local

Si despues de importar una copia aparece un error como `Table 'techos.previsitas' doesn't exist`, primero confirmar que el dump base cargo el esquema completo y no solo tablas aisladas. El listado de seguimiento depende de la tabla base `previsitas`; las migraciones versionadas del repo son incrementales y no reemplazan el dump base completo.

```sql
SELECT TABLE_SCHEMA, COUNT(*) AS tablas
FROM INFORMATION_SCHEMA.TABLES
WHERE TABLE_SCHEMA = 'techos'
GROUP BY TABLE_SCHEMA;

SHOW TABLES LIKE 'previsitas';
SHOW TABLES LIKE 'presupuestos';
```

En Docker local puede ejecutarse con:

```bash
docker exec admintech-db mysql -uroot -proot -D techos -e "SHOW TABLES LIKE 'previsitas'; SHOW TABLES LIKE 'presupuestos';"
```
