...
- Paso 8B de OC agrega precarga asistida del formulario OC desde el PDF subido o desde el PDF guardado en la OC existente: el botón `Analizar PDF y precargar` del bloque Documentación OC llama a la acción `analizar_pdf_orden_compra` en `03-controller/ordenesCompraController.php`, que extrae texto con `smalot/pdfparser` a través de `04-modelo/pdfExtractorModel.php` y combina los datos del PDF con los del circuito (cliente, dirección de obra). Los campos vacíos del formulario se llenan automáticamente; los campos que ya tienen valor no se pisan. Si el PDF no contiene texto extraíble (escaneado o basado en imágenes), se informa al usuario con advertencia y se habilita la carga manual. No se escribe nada en la base de datos durante el análisis; el usuario siempre guarda manualmente.
