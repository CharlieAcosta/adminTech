...
- `03-controller/presupuestosController.php` y `07-funciones_js/presupuestosAcciones.js` mantienen sincronizada la columna `Orden de compra` del listado: al registrar un estado comercial desde el historial se recarga la grilla por el circuito AJAX existente para que `APROBADO` sin OC pase a `OC Pendiente` sin refresh manual. El perfil `Administrativo` no recibe el lapiz tecnico; cuando OC esta habilitada ve un icono propio de gestion OC que abre `seguimiento_form.php` y la vista solo renderiza el acordeon OC. `Administrador` y `Super Administrador` conservan todas las acciones del seguimiento normal y suman un icono OC en la columna Acciones cuando la OC esta habilitada o tiene estado real activo, usando `seguimiento_form.php?acci=e&id={id_previsita}&oc=1#collapse4_OC`.
- `01-views/panel.php` y `01-views/layout/navbar_layout.php` no muestran `Seguimiento de obra` al perfil `Administrativo` si no hay OC pendientes. Para ese perfil, el contador del modulo usa `contarOrdenesCompraPendientes()` y muestra `Pendientes`; los perfiles tecnicos y administradores conservan los contadores tecnicos de vencidas/programadas/reprogramadas y, en el panel principal, suman el badge `OC Pend.` cuando existen OC pendientes, sin mutar titulo/icono/navegacion del modulo.
- Para el perfil `Administrativo`, la presentacion del acceso muta de seguimiento tecnico a bandeja OC: en `01-views/panel.php` el modulo se titula `Ordenes de compra`, usa icono `fa-file-invoice` y el badge dice `Pendientes`, en `01-views/layout/navbar_layout.php` el acceso rapido tambien usa icono `fa-file-invoice` y tooltip `Ordenes de compra`, y en `01-views/seguimiento_de_obra_listado.php` el titulo visible de la pantalla cambia a `Ordenes de compra | Listado`. Otros perfiles conservan `Seguimiento de obra`.
- `01-views/seguimiento_de_obra_listado.php` usa el listado como bandeja OC para `Administrativo`: oculta los filtros de visita/presupuesto y `Agregar`, mantiene solo filtros de tiempo, calcula el rango inicial minimo necesario para incluir todas las OC pendientes, agrega filtros administrativos `Pendientes`/`Cargadas`/`Todas OC` con `Pendientes` por defecto, y `03-controller/presupuestosController.php` abre sesion tambien cuando se invoca por AJAX para leer el perfil real y filtrar server-side solo filas con OC habilitada. En Paso 1, sin persistencia real de OC, `Pendientes` equivale a presupuestos `APROBADO` con estado calculado `OC Pendiente`, `Cargadas` no inventa datos y queda vacio si no hay OC real, y `Todas OC` no muestra `No habilitada`; el buscador de DataTables opera sobre ese dataset ya acotado.
