...
- **PM-AUT-003:** Toda fila de material que entre en autorizacion pendiente muestra controles de decision **Autorizar** y **Rechazar** en la columna **Acciones**. La columna **Autorizacion** queda reservada solo para el badge de estado: `Sin solicitud`, `Solicitada`, `Autorizada` o `Rechazada`. En esta etapa la decision sigue siendo visual/frontend y no existe todavia una entidad ni un actor autorizante de backend; el estado actual se conserva en el snapshot y, al confirmar un pedido, queda congelado en su detalle. No existe accion de revertir: si se autoriza, queda `Autorizada`; si se rechaza, queda `Rechazada`.
