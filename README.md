...
- `01-views/auditoria_configuracion.php` es el subpanel exclusivo de `Super Administrador` para accesos administrativos sensibles; desde el panel principal reemplaza el acceso directo a mail por `Super Administrador` y agrupa `Auditoria` junto con `Configuracion mail presupuestos`. Los accesos directos de cabecera a Auditoria y Mail presupuestos no deben duplicarse fuera de este subpanel.
