...
- `01-views/panel.php` y `01-views/layout/navbar_layout.php` no muestran `Seguimiento de obra` al perfil `Administrativo` si no hay OC pendientes. En el panel principal, el modulo conserva el titulo `Seguimiento de obra` y el icono de seguimiento, pero para ese perfil solo muestra la minicard `Pendientes` calculada con `contarOrdenesCompraPendientes()`; los perfiles tecnicos y administradores conservan los contadores tecnicos de vencidas/programadas/reprogramadas y suman el badge `OC Pend.` cuando existen OC pendientes.
