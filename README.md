...
- Para el perfil `Administrativo`, el acceso rapido de cabecera puede mantener la referencia visual a OC, pero el panel de modulos no muta el boton de `Seguimiento de obra`. Al ingresar, `01-views/seguimiento_de_obra_listado.php` sigue usando la pantalla como bandeja OC administrativa con titulo `Ordenes de compra | Listado`; otros perfiles conservan `Seguimiento de obra`.
- `01-views/seguimiento_de_obra_listado.php` usa el listado como bandeja OC para `Administrativo`: oculta los filtros de visita/presupuesto, muestra `Agregar` para cargar una pre-visita, mantiene solo filtros de tiempo, calcula el rango inicial minimo necesario para incluir todas las OC pendientes, agrega filtros administrativos `Pendientes`/`Cargadas`/`Todas OC` con `Pendientes` por defecto, y `03-controller/presupuestosController.php` abre sesion tambien cuando se invoca por AJAX para leer el perfil real y filtrar server-side solo filas con OC habilitada. En Paso 1, sin persistencia real de OC, `Pendientes` equivale a presupuestos `APROBADO` con estado calculado `OC Pendiente`, `Cargadas` no inventa datos y queda vacio si no hay OC real, y `Todas OC` no muestra `No habilitada`; el buscador de DataTables opera sobre ese dataset ya acotado. En Acciones, `Administrativo` ve solo consulta, edicion de pre-visita y acceso OC; los bloques tecnicos del formulario quedan fuera de su interfaz.
...
- El rango inicial dinamico de la bandeja OC administrativa se resuelve en `04-modelo/ordenCompraWorkflowModel.php`: usa la fecha `created_at` del ultimo evento comercial activo `APROBADO` como fecha de habilitacion de OC cuando existe historial comercial, cae a `previsitas.log_alta` como respaldo, y mapea la antiguedad maxima a `15 dias`, `30 dias`, `Trimestre`, `Semestre`, `Año` o `Todos`. Para que la carga inicial sea coherente, `04-modelo/presupuestosModel.php` usa esa misma fecha de aprobacion como base del filtro de tiempo solo cuando el listado se consulta como bandeja OC administrativa.
...
- En el formulario OC, `Documentacion OC` queda siempre disponible para cargar/reemplazar PDF; el resto de las secciones de datos se bloquea con overlay hasta que exista un PDF guardado o se seleccione un PDF valido en la UI. El guardado sigue pasando por backend, pero la UI advierte con SweetAlert2 si falta PDF o datos principales minimos.
