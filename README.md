...
- Persistencia tecnica del envio por correo:
  ...
  - Tabla nueva: `pedido_materiales_pedido_envios`, con una fila vigente por pedido y tipo, estados `pendiente`, `procesando`, `enviado` o `error`, cantidad de intentos, destinatarios resueltos desde configuracion, documento adjunto, ultimo error saneado y usuario de sesion.
  - Modelo: `04-modelo/pedidoMaterialesEnviosModel.php`, que reclama cada intento en una transaccion corta, reutiliza o genera el PDF backend, valida la configuracion activa, envia por PHPMailer y registra el resultado sin mantener locks durante SMTP.
