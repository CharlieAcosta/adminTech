...
- **PM-PED-006:** Al ejecutar **Realizar pedido**, el pedido activo se confirma en backend como entidad propia antes de avanzar visualmente al siguiente ciclo. La confirmacion se guarda en `pedido_materiales_pedidos` y `pedido_materiales_pedido_detalles`, con cabecera unica por `id_previsita + numero_pedido`, referencia al presupuesto y la Orden de Compra habilitante, usuario de sesion, fecha backend y detalle congelado de las cantidades confirmadas. Esta confirmacion es idempotente: reintentar el mismo pedido con el mismo detalle no lo duplica; si el detalle difiere, se rechaza el avance por conflicto. Esta etapa no genera PDF ni envia correo; esos pasos quedan para PM-MAIL-002/PM-MAIL-003.
