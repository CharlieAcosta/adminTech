...
- **PM-PED-006:** Al ejecutar **Realizar pedido**, el pedido activo se confirma en backend como entidad propia antes de avanzar visualmente al siguiente ciclo. La confirmacion se guarda en `pedido_materiales_pedidos` y `pedido_materiales_pedido_detalles`, con cabecera unica por `id_previsita + numero_pedido`, referencia al presupuesto y la Orden de Compra habilitante, usuario de sesion, fecha backend y detalle congelado de las cantidades confirmadas. Esta confirmacion es idempotente: reintentar el mismo pedido con el mismo detalle no lo duplica; si el detalle difiere, se rechaza el avance por conflicto. PM-PED-006 no genera PDF ni envia correo durante la confirmacion; la generacion posterior del documento corresponde a PM-MAIL-002 y el envio sigue pendiente para PM-MAIL-003.
...
- La pantalla de configuracion de mail de Pedido de Materiales reutiliza el patron visual AdminLTE del panel de Super Administrador y los avisos transitorios del sistema mediante `mostrarExito(...)` / `mostrarError(...)` de `07-funciones_js/funciones.js`; no dispara todavia envios reales ni pruebas SMTP desde la UI.
