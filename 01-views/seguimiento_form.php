...
if (!function_exists('obtenerMaterialesSolicitadosSeguimiento')) {
  function obtenerMaterialesSolicitadosSeguimiento(int $idPrevisita): array
  {
    if ($idPrevisita <= 0) {
      return [];
    }
    ...
    $sql = "
      SELECT
        mv.id_materiales_visita,
        mv.id_visita,
        mv.id_tarea,
        mv.id_material,
        mv.material_cantidad,
        mv.estado,
        m.producto,
        m.marca,
        m.unidad_venta,
        ...
        m.unidad_medida,
        m.rendimiento,
        m.unidad_rendimiento,
        m.descripcion_corta,
        m.estado_material
      FROM materiales_visita mv
      LEFT JOIN materiales m ON m.id_material = mv.id_material
      WHERE mv.id_visita = ?
        AND LOWER(TRIM(COALESCE(mv.estado, 'activo'))) <> 'eliminado'
      ORDER BY mv.id_materiales_visita ASC
    ";
    ...
    $resultado = mysqli_stmt_get_result($stmt);
      ...
      if ($resultado) {
        while ($row = mysqli_fetch_assoc($resultado)) {
          $productoMaterial = trim((string)($row['producto'] ?? ''));
          $descripcionMaterial = trim((string)($row['descripcion_corta'] ?? ''));
          $rows[] = [
            'origen' => 'materiales_visita',
            'origen_label' => 'Adicional',
            'tarea_nro' => 0,
            'tarea_titulo' => 'Material adicional',
            'id_materiales_visita' => $row['id_materiales_visita'] ?? null,
            'id_material' => $row['id_material'] ?? null,
            'nombre_material' => $productoMaterial !== ''
              ? $productoMaterial
              : ($descripcionMaterial !== '' ? $descripcionMaterial : ''),
            'cantidad' => $row['material_cantidad'] ?? '0',
...
if (!function_exists('obtenerMaterialesSolicitadosDesdePresupuestoGenerado')) {
  function obtenerMaterialesSolicitadosDesdePresupuestoGenerado(array $presupuestoGeneradoDetalle): array
  {
    $materiales = [];
    $tareas = $presupuestoGeneradoDetalle['tareas'] ?? [];
    ...
    foreach ($tareas as $tarea) {
      $nroTarea = (int)($tarea['nro'] ?? 0);
      $descripcionTarea = trim(strip_tags((string)($tarea['descripcion'] ?? '')));
      $tituloTarea = $descripcionTarea !== ''
        ? (function_exists('mb_substr') ? mb_substr($descripcionTarea, 0, 80) : substr($descripcionTarea, 0, 80))
        : 'Tarea ' . $nroTarea;

      foreach (($tarea['materiales'] ?? []) as $material) {
        $materiales[] = [
          'origen' => 'presupuesto',
          'origen_label' => 'Presupuesto',
          'tarea_nro' => $nroTarea,
          'tarea_titulo' => $tituloTarea,
          'id_material' => $material['id_material'] ?? null,
          'nombre_material' => $material['nombre_material'] ?? '',
          'cantidad' => $material['cantidad'] ?? '0',
          'unidad_venta' => $material['unidad_venta'] ?? '',
          'unidad_medida' => $material['unidad_medida'] ?? '',
          'precio_unitario_usado' => $material['precio_unitario_usado'] ?? '0',
          'porcentaje_extra' => $material['porcentaje_extra'] ?? '0',
          'subtotal_fila' => $material['subtotal_fila'] ?? '0',
        ];
      }
    }
...
$materialesSolicitadosSeguimiento = [];
$pedidoMaterialesSnapshot = null;
...
if(isset($_GET['id']) && isset($_GET['acci'])){
  ...
  $materialesSolicitadosSeguimiento = obtenerMaterialesSolicitadosSeguimiento((int)($datos['id_previsita'] ?? 0));
...
// START PHP - Visita
      ...
      if(isset($datos['id_previsita'])) {
        ...
        // START PHP PRESUPUESTO GENERADO 
              ...
              $materialesDesdePresupuestoGenerado = obtenerMaterialesSolicitadosDesdePresupuestoGenerado($presupuesto_generado);
              if (!empty($materialesDesdePresupuestoGenerado)) {
                $materialesSolicitadosSeguimiento = array_merge($materialesDesdePresupuestoGenerado, $materialesSolicitadosSeguimiento);
              }
              ...
              $mostrarBloqueOrdenCompraSeguimiento = $accesoOrdenCompraSeguimiento
                && !empty($estadoOrdenCompraSeguimiento['habilitada']);
...
if (isset($datos['id_previsita']) && (int)$datos['id_previsita'] > 0) {
  ...
  $dbPedidoMaterialesSnapshot = conectDB();
  if ($dbPedidoMaterialesSnapshot) {
    mysqli_set_charset($dbPedidoMaterialesSnapshot, 'utf8mb4');
    if (pedidoMaterialesSnapshotTablasMinimasDisponibles($dbPedidoMaterialesSnapshot)) {
      $pedidoMaterialesSnapshot = obtenerPedidoMaterialesSnapshotPorPrevisitaEnConexion(
        $dbPedidoMaterialesSnapshot,
        (int)$datos['id_previsita']
      );
    }
    mysqli_close($dbPedidoMaterialesSnapshot);
  }
}
...
if (
  ...
  $mostrarBloqueOrdenCompraSeguimiento
  && $puedeEditarOrdenCompra
  && !empty($estadoOrdenCompraSeguimiento['habilitada'])
  && (($estadoOrdenCompraSeguimiento['estado'] ?? '') === 'pendiente')
) {
  $previsita_show = '';
  $visita_show = '';
  $presupuesto_show = '';
...
if ($aperturaExclusivaOrdenCompra && !$mostrarBloqueOrdenCompraSeguimiento) {
  $orden_compra_show = '';
  if (!empty($workflowPrevisita['habilita_visita'])) {
    $visita_show = 'show';
  } else {
...
<?php
  ...
  if (!empty($pedidoMaterialesSnapshot['materiales_agregados']) && is_array($pedidoMaterialesSnapshot['materiales_agregados'])) {
    $materialesAgregadosPedido = array_values(array_map(function ($detalleSnapshot) {
      $cantidadInicial = (float)($detalleSnapshot['cantidad_inicial'] ?? 0);
      $pedidosSnapshot = is_array($detalleSnapshot['pedidos'] ?? null) ? $detalleSnapshot['pedidos'] : [];
      $cantidadSolicitada = (float)($detalleSnapshot['cantidad_solicitada'] ?? 0);
      if ($cantidadSolicitada <= 0 && !empty($pedidosSnapshot)) {
        $cantidadSolicitada = array_sum(array_map('floatval', $pedidosSnapshot));
      }

      return [
        'id_material' => (int)($detalleSnapshot['id_material'] ?? 0),
        'nombre_material' => (string)($detalleSnapshot['material_texto'] ?? ''),
        'cantidad' => $cantidadInicial,
        'cantidad_inicial' => $cantidadInicial,
        'cantidad_agregada' => max(0, $cantidadSolicitada - $cantidadInicial),
        'tarea_nro' => (int)($detalleSnapshot['tarea_nro'] ?? 0),
        'tarea_titulo' => (string)($detalleSnapshot['tarea_titulo'] ?? ''),
      ];
    }, $pedidoMaterialesSnapshot['materiales_agregados']));
...
<?php
  $idPrevisitaPedidoMateriales = (int)($datos['id_previsita'] ?? 0);
?>
<script>
...
<div class="accordion <?php echo $pedidoMaterialesHabilitadoInicial ? '' : 'd-none'; ?>" id="accordionExample5" aria-hidden="<?php echo $pedidoMaterialesHabilitadoInicial ? 'false' : 'true'; ?>">
  <div class="card <?php echo $orden_compra_card; ?> accordion 5">
    ...
    <div id="collapse5_PM" class="collapse <?php echo $pedido_materiales_show; ?>"
         data-pedido-materiales-activo="<?php echo (int)($pedidoMaterialesSnapshot['pedido_activo'] ?? 1); ?>"
         data-pedido-materiales-finalizado="<?php echo !empty($pedidoMaterialesSnapshot['finalizado']) ? '1' : '0'; ?>"
      ...
      <div class="card-body">
        ...
        <div class="pedido-materiales-ejecutar-wrapper d-flex justify-content-end mt-3">
          <button type="button"
                  ...
                  disabled>
            <i class="fa fa-save mr-1"></i> Guardar pedido
          </button>
          <button type="button"
  ...
  <div class="card <?php echo $orden_compra_card; ?> accordion 4" id="ordenCompraAccordionCard">
    <div class="card-header" id="heading4">
      <h2 class="mb-0 d-flex align-items-center presupuesto-accordion-header">
        ...
        <span class="card-title text-right presupuesto-accordion-intervino oc-accordion-intervino d-none">
          <strong>Intervino: </strong>
          <span
            class="intervino-oc-ultimo"
            tabindex="0"
            role="button"
            data-toggle="popover"
            data-html="true"
        ...
        <form id="ordenCompraForm" class="orden-compra-form" enctype="multipart/form-data" novalidate>
          ...
          <div class="card card-info mb-3 w-100 oc-card-principal" id="oc_seccion_pdf">
            ...
            <div class="card-body">
              ...
              <div id="oc_estado_alert" class="alert alert-info oc-alert-card">
                <div class="d-flex align-items-start">
                  <span class="oc-alert-icon mr-2"><i class="fas fa-info-circle"></i></span>
                  <div class="oc-alert-content">
              ...
              <div id="oc_pdf_sin_pdf" class="oc-alert-card oc-alert-card-warning<?php echo $ordenCompraPuedeEditarInicial ? '' : ' d-none'; ?>">
                <div class="d-flex align-items-start">
                  <span class="oc-alert-icon mr-2"><i class="fas fa-exclamation-triangle"></i></span>
                  <div>
                    <strong>PDF obligatorio para guardar la OC.</strong>
                    <span class="d-block small mt-1">Cargue primero el PDF de la Orden de Compra. El documento respaldatorio es obligatorio para guardar la OC.</span>
                  </div>
                </div>
              </div>
          ...
          <div class="card card-info mb-3 w-100 oc-card-principal oc-datos-principales-card oc-seccion-bloqueable">
            ...
            <div class="form-row">
              ...
              <div class="form-group col-sm-8 col-xl-4">
            ...
            <div class="form-row">
              ...
              <div class="form-group col-md-2 d-flex align-items-end">
                <div class="custom-control custom-switch mb-2 oc-iva-inline">
          ...
          <div class="card card-info mb-3 w-100 oc-card-principal oc-seccion-bloqueable">
            ...
            <div class="form-row">
              <div class="form-group col-md-4 d-flex align-items-end">
                <div class="custom-control custom-switch mb-2 oc-switch-inline">
          ...
          <div class="card card-info mb-3 w-100 oc-card-principal oc-seccion-bloqueable">
            ...
            <div class="form-row">
              <div class="col-12">
                <div class="oc-subsection-label">Contacto principal en obra</div>
              </div>
              <div class="form-group col-md-5">
            ...
            <div class="form-row">
              <div class="col-12">
                <div class="oc-subsection-label">Contacto alternativo / mantenimiento</div>
              </div>
              <div class="form-group col-md-5">
          ...
          <div class="card card-info mb-3 w-100 oc-card-principal oc-seccion-bloqueable">
            ...
            <div class="form-row">
              <div class="col-12">
                <div class="oc-subsection-label">Compras</div>
              </div>
              <div class="form-group col-md-4">
...
<script>
  window.SEGUIMIENTO_ORDEN_COMPRA = <?php echo jsonParaJsSeguro([
    ...
    'puede_pedido_materiales' => $puedeGestionarPedidoMateriales,
...
<div id="popover-content-oc" style="display:none">
</div>
...
<style>
  ...
  .orden-compra-form .card {
    border: 1px solid #cfd8e3;
    border-radius: 0.4rem;
    box-shadow: 0 2px 6px rgba(20, 35, 50, 0.09);
  }

  .orden-compra-form .card-header {
    border-bottom: 0;
  }

  .orden-compra-form .card-title {
    font-size: 1rem;
    font-weight: 600;
  }
  ...
  .orden-compra-form .input-group-text {
  ...
  .pedido-materiales-card .pedido-materiales-table th,
  .pedido-materiales-card .pedido-materiales-table td {
  ...
  .pedido-materiales-agregar .input-group > .select2-container {
    flex: 1 1 auto;
    min-width: 0;
    width: 1% !important;
  }

  .pedido-materiales-agregar .input-group > .select2-container .select2-selection {
  ...
  .oc-alert-card {
    border-radius: 0.4rem;
    box-shadow: 0 1px 3px rgba(20, 35, 50, 0.06);
    margin-bottom: 1rem;
    padding: 0.72rem 0.85rem;
  }

  .oc-alert-card .oc-alert-icon {
    flex: 0 0 1.35rem;
    font-size: 1rem;
    line-height: 1.25;
    text-align: center;
  ...
  .oc-iva-inline .custom-control-label,
  .oc-switch-inline .custom-control-label {
    margin-bottom: 0;
  }
...
window.initPopoverIntervinoOrdenCompra = function () {
  const $target = $('#heading4 .intervino-oc-ultimo[data-toggle="popover"]');
  if (!$target.length) return;

  window.bindIntervinoPopoverPersistente($target, function () {
    return $('#popover-content-oc').html();
  });
};

window.actualizarIntervinoOrdenCompraUI = function (intervino) {
  const ultimo = String(intervino?.ultimo_texto || '');
  const popoverHtml = String(intervino?.popover_html || '');
  const mostrar = ultimo !== '' && ultimo !== 'Sin intervenciones';

  $('#popover-content-oc').html(popoverHtml);
  $('.intervino-oc-ultimo').text(mostrar ? ultimo : 'Sin intervenciones');
  $('.oc-accordion-intervino').toggleClass('d-none', !mostrar);

  window.initPopoverIntervinoOrdenCompra();
};
  ...
  $('#cp_visita').off('blur');
    ...
    function alertaOrdenCompra(clase, titulo, mensaje) {
      ...
      var iconos = {
        ...
        'alert-warning': 'fas fa-exclamation-circle',
        'alert-secondary': 'fas fa-lock',
        'alert-success': 'fas fa-check-circle',
        'alert-danger': 'fas fa-times-circle',
        'alert-dark': 'fas fa-ban'
      };
      var claseFinal = clase || 'alert-info';
      $alert
        .removeClass('alert-info alert-warning alert-secondary alert-success alert-danger alert-dark')
        .addClass(claseFinal);
      $alert.find('.oc-alert-icon').html('<i class="' + (iconos[claseFinal] || iconos['alert-info']) + '"></i>');
      $alert.find('.oc-alert-content').html('<strong>' + titulo + '</strong>' + (mensaje ? '<span class="d-block">' + mensaje + '</span>' : ''));
    }
    ...
    function ordenCompraEstadoPermiteEdicion() {
      return ['pendiente', 'cargada', 'observada', 'anulada'].indexOf(ordenCompraEstadoActual) !== -1;
    }
    ...
    function actualizarBloqueoSeccionesOrdenCompra() {
      ...
      var puedeEditarPorPermisoYEstado = ordenCompraPuedeEditar && ordenCompraEstadoPermiteEdicion();
      var bloquearPorPdf = puedeEditarPorPermisoYEstado
        && !ordenCompraTienePdfDisponibleParaEditar();
      var bloquearSeccionesDatos = !puedeEditarPorPermisoYEstado || bloquearPorPdf;
      var bloquearSeccionPdf = !puedeEditarPorPermisoYEstado;

      $('#ordenCompraForm .oc-seccion-bloqueable')
        .toggleClass('oc-section-locked', bloquearPorPdf)
        .find('.oc-field')
        .prop('disabled', bloquearSeccionesDatos)
        .prop('readonly', bloquearSeccionesDatos);

      $('#oc_seccion_pdf .oc-field')
        .prop('disabled', bloquearSeccionPdf)
        .prop('readonly', false);
    }
    ...
    function renderPdfOrdenCompra() {
      var puedeCargar = ordenCompraPuedeEditar
    ...
    function setReadonlyOrdenCompra(readonly) {
      if (readonly) {
        $('#ordenCompraForm .oc-field')
          .prop('disabled', true)
          .prop('readonly', true);
        $('#oc_seccion_pdf .oc-field').prop('readonly', false);
        return;
      }
    ...
    function ordenCompraHabilitaPedidoMateriales() {
      return ordenCompraIdActivo > 0 && ['cargada', 'observada'].indexOf(ordenCompraEstadoActual) !== -1;
    }

    function ordenCompraListaParaCarga() {
    ...
    function actualizarAvanceOrdenCompra() {
      ...
      $cardOrdenCompra
        .removeClass('card-danger card-warning card-info card-secondary card-dark')
        .toggleClass('card-success', habilitaPedidoMateriales)
        .toggleClass('card-danger', !habilitaPedidoMateriales);
      ...
      if (habilitaPedidoMateriales && puedePedidoMateriales) {
        ...
        $('#collapseOne, #collapseVisita, #collapsePresupuesto').collapse('hide');
        $('#collapse5_PM').collapse('hide');
    ...
    function renderOrdenCompraPayload(payload) {
      ...
      $('#oc_estado_alert .oc-alert-content strong').append(' <span class="badge ' + badgeClass + ' ml-2">' + label + '</span>');

      if (!ordenCompraPuedeEditar && estado !== 'no_habilitada') {
        $('#oc_estado_alert .oc-alert-content').append('<span class="d-block mt-1">Tu perfil puede consultar este bloque, pero no cargar ni editar la OC.</span>');
      }
    ...
    function formDataOrdenCompraConFlags(accion, flags) {
      var formData = formDataOrdenCompra(accion);
      $.each(flags || {}, function(nombre, valor) {
        formData.append(nombre, valor == null ? '' : valor);
      });
    ...
    function htmlClienteSugeridoOrdenCompra(clienteSugerido) {
      clienteSugerido = clienteSugerido || {};
      var cuit = escapeHtmlOrdenCompra(clienteSugerido.cuit || '');
      var razonSocial = escapeHtmlOrdenCompra(clienteSugerido.razon_social || '');
      var html = '<div class="text-center" style="color:#000000;">';
      html += '<p>Este cliente no existe en el maestro de Clientes. ¿Querés darlo de alta automáticamente con los datos disponibles del circuito?</p>';
      html += '<div class="border rounded bg-light p-2 text-center">';
      html += '<strong>' + (razonSocial || 'Cliente sin razón social') + '</strong>';
      if (cuit) {
        html += '<br><span>CUIT: ' + cuit + '</span>';
      }
      html += '</div>';
      html += '<small class="d-block mt-2 text-center">Si elegís no darlo de alta, la OC se guardará igual y no se modificará Clientes.</small>';
      html += '</div>';

      return html;
    }

    function confirmarAltaClienteOrdenCompra(clienteSugerido) {
      var deferred = $.Deferred();
      var mensajeConfirmacionCliente = 'Este cliente no existe en el maestro de Clientes. \u00bfQuer\u00e9s darlo de alta autom\u00e1ticamente con los datos disponibles del circuito?';

      if (window.Swal && typeof Swal.fire === 'function') {
        Swal.fire({
          icon: 'warning',
          title: '<H2><STRONG style="color:#000000;">Cliente nuevo detectado</STRONG></H2>',
          html: htmlClienteSugeridoOrdenCompra(clienteSugerido),
          background: '#ffc107',
          iconColor: '#000000',
          showCancelButton: true,
          cancelButtonText: 'No, solo guardar OC',
          confirmButtonText: 'S\u00ed, dar de alta cliente',
          confirmButtonColor: '#28a745',
          cancelButtonColor: '#343a40',
          reverseButtons: true
        }).then(function(result) {
          deferred.resolve(!!(result && result.isConfirmed));
        });
        return deferred.promise();
      }

      deferred.resolve(window.confirm(mensajeConfirmacionCliente));
      return deferred.promise();

    }

    function informarResultadoAltaClienteOrdenCompra(resp) {
      var altaCliente = resp && resp.data ? resp.data.cliente_alta : null;
      if (!altaCliente) {
        return false;
      }

      if (altaCliente.accion === 'creado') {
        var cliente = altaCliente.cliente || {};
        var mensaje = 'Cliente creado y OC guardada.';
        if (cliente.razon_social) {
          mensaje = cliente.razon_social + ' fue creado en Clientes y la OC quedó guardada.';
        }
      ...
      if (altaCliente.accion === 'omitida' && altaCliente.motivo === 'datos_insuficientes') {
        toastr.warning('La OC se guardó, pero no se creó cliente porque no hay CUIT o razón social suficiente.');
        return true;
      }

      if (altaCliente.accion === 'omitida' && altaCliente.motivo === 'rechazada_por_usuario') {
        toastr.info('La OC se guardó sin crear cliente.');
        return true;
      }
    ...
    function enviarGuardarOrdenCompra(accion, flags) {
      var deferred = $.Deferred();
      ...
      requestOrdenCompra(formDataOrdenCompraConFlags(accion, flags), { multipart: true }).done(function(resp) {
        ...
        if (resp && resp.requires_cliente_confirmation) {
          confirmarAltaClienteOrdenCompra((resp.data || {}).cliente_sugerido || {}).done(function(confirmado) {
            var flagsReintento = confirmado
              ? { confirmar_alta_cliente: '1' }
              : { guardar_sin_alta_cliente: '1' };

            enviarGuardarOrdenCompra(accion, flagsReintento)
              .done(function(respReintento) { deferred.resolve(respReintento); })
              .fail(function(respReintento) { deferred.reject(respReintento); });
          });
          return;
        }
    ...
    function confirmarAnularOrdenCompra() {
      if (!ordenCompraPuedeEditar || ordenCompraIdActivo <= 0) {
        return;
      }
      ...
      if (window.Swal && typeof Swal.fire === 'function') {
        Swal.fire({
          ...
          text: 'La OC dejara de contar como activa. Si el presupuesto sigue aprobado, volvera a quedar pendiente.',
          showCancelButton: true,
      ...
      if (window.confirm('Anular la Orden de compra?')) {
        cambiarEstadoOrdenCompra('anulada');
      }
    }
    ...
    function inicializarSelectPedidoMateriales() {
      var $select = $('#pedido_material_select');
      $select
        .removeClass('v-visual-style')
        .prop('disabled', false)
        .prop('readonly', false);

      $('#pedido_material_cantidad')
        .removeClass('v-visual-style')
        .prop('disabled', false)
        .prop('readonly', false);

      $('#pedido_material_agregar')
        .removeClass('v-visual-style')
        .prop('disabled', false);
      ...
      if ($select.find('option').length <= 1 && $('#opcionesMaterialBase').length) {
        $select.html($('#opcionesMaterialBase').html());
      }

      filtrarSelectPedidoMaterialesDisponibles();
    ...
    $('#collapse5_PM').on('shown.bs.collapse', inicializarSelectPedidoMateriales);
    setTimeout(inicializarSelectPedidoMateriales, 0);

    function obtenerIdsMaterialesPedidoNoSeleccionables() {
      var ids = {};

      $('#pedidoMaterialesPresupuestadosTableBody tr, #pedidoMaterialesAgregadosTableBody tr').each(function() {
        var materialId = String($(this).data('material-id') || '');
        if (materialId) {
          ids[materialId] = true;
        }
      });
    ...
    function filtrarSelectPedidoMaterialesDisponibles() {
      var $select = $('#pedido_material_select');
      if (!$select.length) {
        return;
      }

      var idsNoSeleccionables = obtenerIdsMaterialesPedidoNoSeleccionables();
      $select.find('option').each(function() {
        var valor = String($(this).val() || '');
        if (valor && idsNoSeleccionables[valor]) {
          $(this).remove();
        }
      });
    }

    function quitarMaterialPedidoDelSelect(materialId) {
      var $select = $('#pedido_material_select');
      if (!$select.length) {
        return;
      }
      ...
      $select.find('option').filter(function() {
        return String($(this).val() || '') === String(materialId || '');
      }).remove();

      $select.val(null).trigger('change.select2');
    }

    function materialPedidoEstaPresupuestado(materialId) {
      return obtenerFilasPedidoMaterialPorId('#pedidoMaterialesPresupuestadosTableBody', materialId).length > 0;
    }

    function agregarMaterialPedidoAlSelect(materialId, materialTexto) {
      var $select = $('#pedido_material_select');
      if (!$select.length || !materialId || materialPedidoEstaPresupuestado(materialId)) {
        return;
      }

      if (obtenerFilasPedidoMaterialPorId('#pedidoMaterialesAgregadosTableBody', materialId).length > 0) {
        return;
      }

      var existeOpcion = $select.find('option').filter(function() {
        return String($(this).val() || '') === String(materialId || '');
      }).length > 0;

      if (!existeOpcion) {
        $select.append(new Option(materialTexto || ('Material #' + materialId), materialId, false, false));
      }
    ...
    function obtenerFilasPedidoMaterialPorId(selector, materialId) {
      return $(selector).find('tr').filter(function() {
        return String($(this).data('material-id') || '') === String(materialId || '');
      });
    }

    function obtenerCantidadPedidoMaterialDesdeFila($fila, atributo, selectorBadge) {
      var textoCantidad = String($fila.attr(atributo) || $fila.find(selectorBadge).first().text() || '0');
      var cantidad = parseFloat(textoCantidad.replace(/\./g, '').replace(',', '.'));
    ...
    function normalizarCantidadSnapshotPersistida(valor) {
      ...
      if (/^-?\d+\.\d{1,6}$/.test(texto)) {
        var decimal = Number(texto);
        return isFinite(decimal) && decimal > 0 ? decimal : 0;
      }

      return parsearCantidadPedidoMaterial(texto);
    }
    ...
    function obtenerEndpointPedidoMateriales() {
      return String(
        pedidoMaterialesConfig.url_guardar_snapshot
    ...
    function actualizarEstadoBotonGuardarPedidoMateriales() {
      var $boton = $('#pedido_materiales_guardar');
      if (!$boton.length) {
        return;
      }
      ...
      $boton
        ...
        .toggleClass('btn-info', botonHabilitado)
        .toggleClass('btn-secondary', !botonHabilitado);
    }

    function marcarPedidoMaterialesConCambiosPendientes() {
      if (pedidoMaterialesRestaurandoSnapshot) {
        return;
      }

      pedidoMaterialesTieneCambiosPendientes = true;
      actualizarEstadoBotonGuardarPedidoMateriales();
    }

    function limpiarCambiosPendientesPedidoMateriales() {
      pedidoMaterialesTieneCambiosPendientes = false;
      actualizarEstadoBotonGuardarPedidoMateriales();
    }
    ...
    function obtenerNumeroMaximoPedidoMaterialesVisible() {
      var numeroMaximo = 1;

      $('#collapse5_PM .pedido-materiales-pedido-header').each(function() {
        var numeroPedido = parseInt(String($(this).attr('data-pedido-numero') || '0'), 10);
        if (numeroPedido > numeroMaximo) {
          numeroMaximo = numeroPedido;
        }
      });

      return normalizarNumeroPedidoMateriales(numeroMaximo);
    }
    ...
    function obtenerBadgePedidoMaterialEnFila($fila, numeroPedido) {
      return $fila.find('.pedido-materiales-pedido[data-pedido-numero="' + numeroPedido + '"]').first();
    }

    function obtenerCantidadPedidoMaterialEnFila($fila, numeroPedido) {
      var $badgePedido = obtenerBadgePedidoMaterialEnFila($fila, numeroPedido);

      return parsearCantidadPedidoMaterial($badgePedido.find('strong').first().text());
    }

    function obtenerCantidadPedidoActivoMaterialEnFila($fila) {
      return obtenerCantidadPedidoMaterialEnFila($fila, obtenerNumeroPedidoMaterialesActivo());
    }

    function obtenerCantidadTotalPedidosMaterialEnFila($fila) {
      var total = 0;

      $fila.find('.pedido-materiales-pedido strong').each(function() {
        total += parsearCantidadPedidoMaterial($(this).text());
      });

      return total;
    }

    function obtenerCantidadPedidosCongeladosMaterialEnFila($fila) {
      var numeroPedidoActivo = obtenerNumeroPedidoMaterialesActivo();
      var total = 0;

      $fila.find('.pedido-materiales-pedido').each(function() {
        var numeroPedido = parseInt(String($(this).attr('data-pedido-numero') || '0'), 10);

        if (numeroPedido > 0 && numeroPedido !== numeroPedidoActivo) {
          total += parsearCantidadPedidoMaterial($(this).find('strong').first().text());
        }
      });
    ...
    function requiereAutorizacionPedidoMaterial($fila, cantidadPedidoActivoPropuesta) {
      var totalAcumuladoPropuesto = obtenerCantidadTotalPedidosIncluyendoPropuesta($fila, cantidadPedidoActivoPropuesta);

      if ($fila.closest('#pedidoMaterialesAgregadosTableBody').length > 0) {
        return totalAcumuladoPropuesto > 0;
      }
    ...
    function existeColumnaPedidoMaterial(numeroPedido) {
      return $('.pedido-materiales-pedido-header[data-pedido-numero="' + numeroPedido + '"]').length > 0;
    }
    ...
    function actualizarAgregadoPedidoMaterialEnFila($fila, cantidadSumar) {
      ...
      actualizarCantidadPedidoMaterialEnFila($fila, numeroPedidoActivo, cantidadNueva);
    ...
    function obtenerCantidadSolicitadaPedidoMaterialEnFila($fila) {
      return Math.max(0, obtenerCantidadTotalPedidosMaterialEnFila($fila));
    }
    ...
    function destruirTooltipsAccionesPedidoMateriales($contexto) {
      var $tooltips = $contexto.find('[data-toggle="tooltip"]');
    ...
    function limpiarTooltipsPedidoMaterialEnFila($fila) {
      destruirTooltipsAccionesPedidoMateriales($fila);
    }
    ...
    function renderizarAccionesAutorizacionPedidoMaterial($fila, estado) {
      ...
      inicializarTooltipsAccionesPedidoMateriales($contenedor);
    }
    ...
    function obtenerCantidadPedidoVisualMaterialEnFila($fila, solicitadoVisual) {
      return obtenerCantidadPedidoActivoMaterialEnFila($fila);
    }
    ...
    function actualizarEstadoBotonRealizarPedido() {
      ...
      $('#pedidoMaterialesPresupuestadosTableBody tr[data-material-id], #pedidoMaterialesAgregadosTableBody tr[data-material-id]').each(function() {
        var cantidadPedido = obtenerCantidadPedidoMaterialEnFila($(this), numeroPedidoActivo);
    ...
    function obtenerDetallePedidoMaterialesParaDeposito() {
      ...
      $('#pedidoMaterialesPresupuestadosTableBody tr[data-material-id], #pedidoMaterialesAgregadosTableBody tr[data-material-id]').each(function() {
        var $fila = $(this);
        var cantidadPedido = obtenerCantidadPedidoMaterialEnFila($fila, numeroPedidoActivo);
    ...
    function obtenerSnapshotFilaPedidoMaterial($fila, tipoFila, ordenVisual) {
      var pedidos = {};
      var numeroPedido;

      for (numeroPedido = 1; numeroPedido <= PEDIDO_MATERIALES_MAXIMO; numeroPedido += 1) {
        pedidos[numeroPedido] = obtenerCantidadPedidoMaterialEnFila($fila, numeroPedido);
      }
      ...
      return {
        ...
        tarea_titulo: String($fila.attr('data-tarea-titulo') || '').trim(),
        id_material: parseInt(String($fila.attr('data-material-id') || '0'), 10) || 0,
        material_texto: String($fila.attr('data-material-text') || '').trim(),
        cantidad_inicial: obtenerCantidadPedidoMaterialDesdeFila($fila, 'data-material-cantidad-inicial', '.pedido-materiales-cantidad-inicial strong'),
        cantidad_solicitada: obtenerCantidadSolicitadaPedidoMaterialEnFila($fila),
        pedidos: pedidos,
    ...
    function obtenerSnapshotFilasPedidoMateriales(selector, tipoFila) {
      var filas = [];

      $(selector).find('tr[data-material-id]').each(function(indice) {
        filas.push(obtenerSnapshotFilaPedidoMaterial($(this), tipoFila, indice + 1));
      });

      return filas;
    }

    function obtenerSnapshotPedidoMateriales() {
      return {
        id_previsita: parseInt(String(pedidoMaterialesConfig.id_previsita || '0'), 10) || 0,
        pedido_activo: obtenerNumeroPedidoMaterialesActivo(),
        pedido_maximo_visible: obtenerNumeroMaximoPedidoMaterialesVisible(),
        finalizado: pedidoMaterialesEstaFinalizado(),
        materiales_presupuestados: obtenerSnapshotFilasPedidoMateriales('#pedidoMaterialesPresupuestadosTableBody', 'presupuestado'),
        materiales_agregados: obtenerSnapshotFilasPedidoMateriales('#pedidoMaterialesAgregadosTableBody', 'agregado')
      };
    }

    function guardarSnapshotPedidoMateriales(accionGuardado) {
      pedidoMaterialesGuardandoSnapshot = true;
      actualizarEstadoBotonGuardarPedidoMateriales();
      ...
      return $.ajax({
        ...
        dataType: 'json',
        data: JSON.stringify({
          accion: 'guardar_snapshot',
          snapshot: $.extend({}, obtenerSnapshotPedidoMateriales(), {
            accion_guardado: accionGuardado === 'realizar' ? 'realizar' : 'guardar'
          })
        })
      }).then(function(resp) {
        if (!resp || resp.success === false) {
          var mensajeError = (resp && resp.message) || (resp && resp.error) || 'No se pudo guardar el pedido de materiales.';
        ...
        pedidoMaterialesGuardandoSnapshot = false;
        actualizarEstadoBotonGuardarPedidoMateriales();
        actualizarEstadoBotonRealizarPedido();
      });
    }
    ...
    function reiniciarAutorizacionPedidoMaterialParaNuevoCiclo($fila) {
      $fila
        .removeAttr('data-material-autorizacion-adicional data-material-autorizacion-pedido-previo')
        .attr('data-material-autorizacion-estado', 'sin_solicitud');

      renderizarBadgeAutorizacionPedidoMaterial($fila, 'sin_solicitud');
    ...
    $(document).on('click', '#pedido_materiales_guardar', function() {
      if ($(this).prop('disabled') || !pedidoMaterialesTieneCambiosPendientes) {
        return;
      }

      guardarSnapshotPedidoMateriales('guardar').then(function(resp) {
        if (!resp || resp.success === false) {
          throw resp;
        }
    ...
    function actualizarResumenVisualPedidoMaterialEnFila($fila) {
      ...
      actualizarCantidadPedidoMaterialEnFila($fila, obtenerNumeroPedidoMaterialesActivo(), pedidoVisual);
    ...
    function marcarAutorizacionSolicitadaPedidoMaterialEnFila($fila, cantidadSolicitadaAutorizacion) {
      ...
      actualizarCantidadPedidoMaterialEnFila($fila, obtenerNumeroPedidoMaterialesActivo(), pedidoActivoPropuesto);
    ...
    function cambiarEstadoAutorizacionPedidoMaterialEnFila($fila, estado) {
      ...
      if (estado === 'rechazada') {
        ...
        $fila.removeAttr('data-material-autorizacion-pedido-previo');
      }
    ...
    function aplicarSnapshotDetallePedidoMaterialAFila($fila, detalleSnapshot, numeroMaximoVisible) {
      var pedidos = (detalleSnapshot && detalleSnapshot.pedidos) || {};
      var estadoAutorizacion = (detalleSnapshot && detalleSnapshot.estado_autorizacion) || 'sin_solicitud';
      var numeroPedido;

      if (!$fila.length || !detalleSnapshot) {
        return;
      }

      $fila.attr({
        'data-material-id': String(detalleSnapshot.id_material || 0),
        'data-material-text': String(detalleSnapshot.material_texto || ''),
        'data-material-cantidad-inicial': String(normalizarCantidadSnapshotPersistida(detalleSnapshot.cantidad_inicial)),
      ...
      if (detalleSnapshot.autorizacion_adicional !== null && detalleSnapshot.autorizacion_adicional !== undefined) {
        $fila.attr('data-material-autorizacion-adicional', String(normalizarCantidadSnapshotPersistida(detalleSnapshot.autorizacion_adicional)));
      } else {
        $fila.removeAttr('data-material-autorizacion-adicional');
      }

      if (detalleSnapshot.pedido_autorizacion_previo !== null && detalleSnapshot.pedido_autorizacion_previo !== undefined) {
        $fila.attr('data-material-autorizacion-pedido-previo', String(normalizarCantidadSnapshotPersistida(detalleSnapshot.pedido_autorizacion_previo)));
      } else {
        $fila.removeAttr('data-material-autorizacion-pedido-previo');
      }

      for (numeroPedido = 1; numeroPedido <= numeroMaximoVisible; numeroPedido += 1) {
        if (numeroPedido > 1) {
          agregarColumnaPedidoMaterial(numeroPedido);
          agregarCeldaPedidoMaterialSiFalta($fila, numeroPedido, 0);
        }

        actualizarCantidadPedidoMaterialEnFila(
          $fila,
          numeroPedido,
          normalizarCantidadSnapshotPersistida(pedidos[numeroPedido] || pedidos[String(numeroPedido)] || 0)
        );
      }
    ...
    function restaurarSnapshotPedidoMateriales(snapshot) {
      var snapshotPedidoMateriales = snapshot || window.PEDIDO_MATERIALES_SNAPSHOT;
      var numeroMaximoVisible;
      var numeroPedidoActivo;
      var numeroPedido;

      if (!snapshotPedidoMateriales || typeof snapshotPedidoMateriales !== 'object') {
        return;
      }

      pedidoMaterialesRestaurandoSnapshot = true;

      try {
        numeroMaximoVisible = normalizarNumeroPedidoMateriales(snapshotPedidoMateriales.pedido_maximo_visible || 1);
        numeroPedidoActivo = normalizarNumeroPedidoMateriales(snapshotPedidoMateriales.pedido_activo || 1);

        for (numeroPedido = 2; numeroPedido <= numeroMaximoVisible; numeroPedido += 1) {
          agregarColumnaPedidoMaterial(numeroPedido);
        }
        ...
        $('#collapse5_PM').attr('data-pedido-materiales-finalizado', snapshotPedidoMateriales.finalizado ? '1' : '0');

        for (numeroPedido = 1; numeroPedido <= numeroMaximoVisible; numeroPedido += 1) {
          if (numeroPedido < numeroPedidoActivo || snapshotPedidoMateriales.finalizado) {
            congelarPedidoMateriales(numeroPedido);
          }
        }
        ...
        if (snapshotPedidoMateriales.finalizado) {
          finalizarFlujoPedidoMateriales();
        } else {
          $('#pedido_material_select, #pedido_material_cantidad, #pedido_material_agregar').prop('disabled', false);
          actualizarEstadoBotonRealizarPedido();
        }
      } finally {
        pedidoMaterialesRestaurandoSnapshot = false;
        limpiarCambiosPendientesPedidoMateriales();
